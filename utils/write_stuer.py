...
def write_stuer(table: str, file: str, engine: Engine) -> None:
    '''
    Description:
        Truncates the target table and writes classroom data from a
        CSV file into it.
    ...
    Args:
        ...
        file (str): Full path to the CSV file containing classroom data.
    ...
    df = df[[
        'Id', 'Navn', 'Institution',
        'StartAlder', 'SlutAlder', 'Aldersgruppe', 'Arkiveret']]
    
    df.columns = [
        'id', 'name', 'institution_id',
        'start_age_in_months', 'end_age_in_months', 'age_group', 'archived']
    
    with engine.begin() as conn:
        conn.execute(text(f'DELETE FROM {table}'))
        df.to_sql(table, con=conn, if_exists='append', index=False)
    logger.info('write_stuer completed')